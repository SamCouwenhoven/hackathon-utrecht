...
public class GameScreen implements Screen {
    ...
    Animation<TextureRegion> bucketAnimation;
    long lastDropTime;
    int dropsGathered;
    ...
    public GameScreen(final Drop game) {
        ...
        bucketAnimation = new Animation<TextureRegion>(0.000006f, walkFrames);
        ...
        player2.x = 800 / 2 - 64 / 2; // center the bucket horizontally
        player2.y = 20; // bottom left corner of the bucket is 20 pixels above
    ...
    public void render(float delta) {
        ...
        game.batch.draw(bucketAnimation.getKeyFrame(delta,true), bucket.x, bucket.y, bucket.width, bucket.height);
        game.batch.draw(bucketAnimation.getKeyFrame(delta,true), player2.x, player2.y, player2.width, player2.height);
        ...
        if (Gdx.input.isKeyPressed(Keys.A))
            player2.x -= 200 * Gdx.graphics.getDeltaTime();
        if (Gdx.input.isKeyPressed(Keys.D))
            player2.x += 200 * Gdx.graphics.getDeltaTime();
        if (Gdx.input.isKeyPressed(Keys.S))
            player2.y -= 200 * Gdx.graphics.getDeltaTime();
        ...
        while (iter.hasNext()) {
            ...
            if (raindrop.y + 64 < 0)
                iter.remove();
            if (raindrop.overlaps(bucket) || raindrop.overlaps(player2)) {
                dropsGathered++;
                dropSound.play();
                iter.remove();
            }
